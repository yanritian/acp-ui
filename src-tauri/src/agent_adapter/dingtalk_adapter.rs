...
impl DingTalkAdapter {
    ...
    /// 获取审批列表
    pub async fn get_approval_list(
        &self,
        process_code: &str,
        status: &str,
    ) -> Result<AgentResult, AgentError> {
...
impl AgentAdapter for DingTalkAdapter {
    ...
    fn capabilities(&self) -> Vec<Capability> {
        vec![
            Capability {
                name: "dingtalk_oauth".to_string(),
                proficiency: 0.95,
                cost_per_unit: 0.0,
                latency_ms: 1000,
            },
            Capability {
                name: "dingtalk_message".to_string(),
                proficiency: 0.90,
                cost_per_unit: 0.0,
                latency_ms: 1500,
            },
            Capability {
                name: "dingtalk_approval".to_string(),
                proficiency: 0.85,
                cost_per_unit: 0.0,
                latency_ms: 2000,
            },
            Capability {
                name: "dingtalk_group".to_string(),
                proficiency: 0.90,
                cost_per_unit: 0.0,
                latency_ms: 1000,
            },
            Capability {
                name: "dingtalk_schedule".to_string(),
                proficiency: 0.80,
                cost_per_unit: 0.0,
                latency_ms: 1500,
            },
    ...
    async fn configure(&mut self, config: AgentConfig) -> Result<(), AgentError> {
        let app_key = config.metadata.get("app_key").cloned().ok_or_else(|| {
            AgentError::ConfigurationError {
                message: "缺少 app_key".to_string(),
            }
        })?;
    ...
    async fn execute(&self, task: AgentTask) -> Result<AgentResult, AgentError> {
        ...
        let action = match task.description.as_str() {
            ...
            s if s.contains("消息") || s.contains("发送") || s.contains("通知") => {
        ...
        let result = match action {
            DingTalkAction::Auth => AgentResult {
            ...
            DingTalkAction::SendMessage => {
                ...
                self.send_work_notice(&receivers, &msg_type, &content)
                    .await?
            }
            DingTalkAction::CreateApproval => {
                ...
                self.create_approval(&process_code, &title, &user_id, &HashMap::new())
                    .await?
            }
            ...
            DingTalkAction::ScheduleManage => AgentResult {
                task_id: task.id.clone(),
                agent_id: self.id.clone(),
                status: ResultStatus::Success,
                output: TaskOutput::Text(
            ...
            DingTalkAction::GetUserinfo => AgentResult {
                task_id: task.id.clone(),
                agent_id: self.id.clone(),
                status: ResultStatus::Success,
                output: TaskOutput::Text(
