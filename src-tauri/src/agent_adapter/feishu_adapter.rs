...
impl AgentAdapter for FeishuAdapter {
    ...
    fn capabilities(&self) -> Vec<Capability> {
        vec![
            Capability {
                name: "feishu_oauth".to_string(),
                proficiency: 0.95,
                cost_per_unit: 0.0,
                latency_ms: 1000,
            },
            Capability {
                name: "feishu_message".to_string(),
                proficiency: 0.90,
                cost_per_unit: 0.0,
                latency_ms: 1200,
            },
            Capability {
                name: "feishu_document".to_string(),
                proficiency: 0.85,
                cost_per_unit: 0.0,
                latency_ms: 2000,
            },
            ...
            Capability {
                name: "feishu_folder".to_string(),
                proficiency: 0.95,
                cost_per_unit: 0.0,
                latency_ms: 1000,
            },
            Capability {
                name: "feishu_schedule".to_string(),
                proficiency: 0.80,
                cost_per_unit: 0.0,
                latency_ms: 1500,
            },
    ...
    async fn configure(&mut self, config: AgentConfig) -> Result<(), AgentError> {
        ...
        self.config = Some(FeishuConfig {
            app_id,
            app_secret,
            tenant_access_token: tenant_access_token.clone(),
            user_access_token,
    ...
    async fn execute(&self, task: AgentTask) -> Result<AgentResult, AgentError> {
        ...
        let action = match task.description.as_str() {
            ...
            s if s.contains("多维表格") || s.contains("bitable") || s.contains("表格") => {
        ...
        let result = match action {
            ...
            FeishuAction::GetDocument => AgentResult {
                task_id: task.id.clone(),
                agent_id: self.id.clone(),
                status: ResultStatus::Success,
                output: TaskOutput::Text(
            ...
            FeishuAction::ScheduleManage => AgentResult {
                task_id: task.id.clone(),
                agent_id: self.id.clone(),
                status: ResultStatus::Success,
                output: TaskOutput::Text(
            ...
            FeishuAction::GetUserInfo => AgentResult {
                task_id: task.id.clone(),
                agent_id: self.id.clone(),
                status: ResultStatus::Success,
                output: TaskOutput::Text(
