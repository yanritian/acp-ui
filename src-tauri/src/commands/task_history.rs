...
#[tauri::command]
pub fn search_tasks(keyword: String, limit: Option<u64>, state: State<AppState>) -> Result<Vec<database::TaskRecord>, String> {
    let db = state.database.lock().map_err(|e| format!("Database lock error: {}", e))?;
    let db = db.as_ref().ok_or_else(|| "Database not initialized".to_string())?;
...
#[tauri::command]
pub fn get_task_detail(task_id: String, state: State<AppState>) -> Result<Option<database::TaskRecord>, String> {
