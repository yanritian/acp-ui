...
impl GodotProjectAnalyzer {
    ...
    pub fn analyze_project(path: &Path) -> Result<GodotProjectInfo, GodotAnalyzerError> {
        ...
        let project_name =
            Self::extract_project_name(&project_content).unwrap_or_else(|| "Unknown".to_string());
    ...
    fn extract_main_scene(content: &str) -> Option<String> {
        for line in content.lines() {
            ...
            if line.starts_with("run/main_scene=") {
                return Some(
                    line.trim_start_matches("run/main_scene=")
                        .trim_matches('"')
                        .to_string(),
                );
            }
    ...
    fn find_files_with_extension(
        base_path: &Path,
        extensions: &[&str],
    ) -> Result<Vec<PathBuf>, GodotAnalyzerError> {
    ...
    fn find_asset_files(base_path: &Path) -> Result<Vec<PathBuf>, GodotAnalyzerError> {
        let asset_extensions = [
            "png", "jpg", "jpeg", "svg", "wav", "mp3", "ogg", "tres", "res",
        ];
