...
impl FunctionFlowTester {
    ...
    fn record(&mut self, step: &str, input: &str, output: &str, success: bool, error: Option<String>) {
    ...
    fn report(&self) -> String {
        ...
        for r in &self.records {
            report.push_str(&format!(
                "【步骤 {}】{}\n",
                self.records.iter().position(|x| x.step == r.step).unwrap_or(0) + 1,
                r.step
            ));
...
fn test_one_shot_flow() {
    ...
    tester.record(
        ...
        if detected_role != "developer" { Some("期望 developer，实际 ".to_string() + &detected_role) } else { None }
    );
    ...
    tester.record(
        ...
        if detected_scene != "code_generation" { Some("期望 code_generation".to_string()) } else { None }
    );
    ...
    tester.record(
        ...
        if selected_agent != "claude-code" { Some("期望 claude-code".to_string()) } else { None }
    );
    ...
    tester.record(
        "透明度生成",
        &format!("role={}, scene={}, agent={}", detected_role, detected_scene, selected_agent),
        &transparency,
        true,
        None
    );
...
fn test_privacy_flow() {
    ...
    tester.record("任务输入", task_content, &format!("文件: {:?}", file_paths), true, None);
    ...
    tester.record(
        ...
        if sensitive_detected.is_empty() { Some("未检测到敏感数据".to_string()) } else { None }
    );
    ...
    tester.record(
        "隐私级别判定",
        &format!("敏感={}, 文件={}", sensitive_detected.len(), file_paths.len()),
        &format!("隐私级别: {}", privacy_level),
        privacy_level == "Sanitized",
        if privacy_level != "Sanitized" { Some("期望 Sanitized".to_string()) } else { None }
    );
    ...
    tester.record(
        ...
        if !sanitized.contains("[REDACTED") { Some("脱敏失败".to_string()) } else { None }
    );
...
fn test_goal_graph_flow() {
    ...
    tester.record(
        "创建 Goal B",
        "实现登录API（依赖 goal-a）",
        &format!("Goal ID: {}, Dependencies: {:?}", goal_b.id, goal_b.dependencies),
    ...
    tester.record(
        "获取就绪 Goal",
        "检查依赖状态",
        &format!("就绪 Goal: {:?}", ready_goals.iter().map(|g| &g.id).collect::<Vec<_>>()),
    ...
    tester.record(
        "重新检查就绪 Goal",
        "Goal A 已收敛",
        &format!("就绪 Goal: {:?}", ready_goals_after.iter().map(|g| &g.id).collect::<Vec<_>>()),
        ready_goals_after.len() == 1 && ready_goals_after[0].id == "goal-b",
        if ready_goals_after.iter().any(|g| g.id == "goal-b") { None } else { Some("Goal B 未能就绪".to_string()) }
    );
...
fn select_agent(role: &str, scene: &str) -> (String, String) {
    if scene == "code_generation" {
        ("claude-code".to_string(), "Selected claude-code for code generation".to_string())
    } else if scene == "bug_fix" {
        ("codex".to_string(), "Selected codex for bug fix".to_string())
    } else if role == "marketer" {
        ("kimi".to_string(), "Selected kimi for marketing".to_string())
    } else {
...
fn get_ready_goals(goals: &[Goal]) -> Vec<Goal> {
    goals.iter()
        .filter(|g| g.status == "Pending" && g.dependencies.iter().all(|dep| {
            goals.iter().any(|other| other.id == *dep && other.status == "Converged")
        }))
        .cloned()
        .collect()
}
